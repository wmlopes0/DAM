...
public class Escritura {
    ...
    private static void escribirInformeOficiales(PrintWriter pw, ArrayList<Personaje> personajes) {
        for (Personaje personaje : personajes) {
            if (personaje instanceof Oficial) {
                ((Oficial) personaje).mostrarCeldasCatalogados(pw);
            }
        }
    }

    //ESCRITURA INFORME OFICIALES (DEFENSA)
    public static void escribirInformeCeldasCatalogadas() {
        //Patrón singleton
        CentralNuclear central = CentralNuclear.getInstancia();
        //Obtengo la lista de personajes de la central
        ArrayList<Personaje> personajes = central.getlPersonajes();
        ...
        dirInformes.mkdir();
        //Genero el nombre del fichero
        String nombreFichero = Utilidad.generarNombreFichero(Constantes.FICHERO_INFORME_OFICIALES);
        File ficheroInforme = new File(nombreFichero);
        //Escribo
        FileWriter fw = null;
        PrintWriter pw = null;
        ...
        try {
            fw = new FileWriter(ficheroInforme);
            pw = new PrintWriter(fw);
            //Escribir informe Oficiales
            escribirInformeOficiales(pw, personajes);
        } catch (IOException ex) {
            System.out.println(Constantes.ERROR_ENTRADA_SALIDA);
        } finally {
            if (pw != null) {
